...
// ─────────────────────────────────────────────────────────────────────────────
// GQA = 2 native variants (fp16 KV).
//
// Same work-group geometry as the GQA=4 kernels — 4 threads in phase1 (one per
// 64-col head_dim segment), 2 threads in phase2 (one per 32-token segment) — but
// each work-group now owns ONLY 2 q-heads (one kv-head's full group) instead of 4.
// The head_dim / token tiling and all simd widths are byte-for-byte identical to
// the GQA=4 path; the only differences are:
//   * q-head loop bounds 4 -> 2  (qn / pn / oc)
//   * q-head mapping qHeadIdx = kvHeadIdx * gqaRatio   (gqaRatio == 2 here)
//   * in the softmax/output stage the surplus threads (hh >= 2 in phase1,
//     hh >= 1 in phase2) return after the barrier — there is no 3rd/4th q-head.
// This computes exactly 2 q-heads of QK·PV, i.e. half the work of padding to 4.
// Phase3 is reused unchanged (it is per-q-head, gqa-agnostic).
ESIMD_INLINE void sdpaDecodeGqa2Phase1(
  ...
  int qHeadIdx = kvHeadIdx * gqaRatio;               // 2 q-heads start here
  uint32_t pageTableSize = 1 << pageTableSizeLog2;
  uint32_t pageTableLoopMask = pageTableSize - 1;
  uint32_t kvSeqLen = batchKvSeqLen[batchIdx];
  uint32_t pageTableBase = pageTableBatchStride * batchIdx;
  ...
  simd<uint32_t, 16> baseOffsetInc16AsSimd(baseOffsetInc16);
  simd<float, 64 * QPER> qqFp32;
  simd<fp16, 64 * QPER> qqFp16;
  simd<float, 16 * QPER> ppFp32;
  simd<fp16, 16 * 64> kkCache;
  simd<float, 1> ppMax;
...
#pragma unroll
  ...
  // Only the first QPER threads correspond to real q-heads; the rest exit.
  if (hh >= QPER) {
    return;
  }
...
#pragma unroll
  ...
  // GQA=2: only thread hh==0 owns real q-heads (0,1); hh==1 has no (2,3) pair.
  if (hh >= 1) {
    return;
  }

  {
    // dividor holds both threads' token-partial softmax sums for the 2 q-heads:
    //   [qhead0_t0, qhead0_t1, qhead1_t0, qhead1_t1]; sum the pairs (same as GQA=4).
    simd<float, 4> dividor;
    simd<float, QPER> softmaxMul;
    simd<float, QPER * 32> outputTempFp32;
    dividor = slm_block_load<float, 4>(slmSizeOut);          // hh==0 here
    softmaxMul = dividor.select<2, 2>(0) + dividor.select<2, 2>(1);
    outputTempFp32 = slm_block_load<float, QPER * 32>(0);    // hh==0 here
