...
class GPUModelRunner(
    ...
    def _prepare_input_ids(
        ...
        if isinstance(self._draft_token_ids, list):
            padded = torch.zeros(
                len(self._draft_token_ids), self.num_spec_tokens,
                dtype=torch.int32, pin_memory=self.pin_memory,
            )
            for i, toks in enumerate(self._draft_token_ids):
                if toks:
                    padded[i, :len(toks)] = torch.tensor(toks, dtype=torch.int32)
    ...
    def propose_draft_token_ids(
        ...
        if spec_config.method == "ngram":
            ...
            if (
                ...
                # Eager forwards return a fresh list, so release the copied
                # source tensors before DFlash builds context KV.
                aux_hidden_states.clear()
