...
static c10::OperatorHandle find_op(const char* name, const char* overload = "") {
    return c10::Dispatcher::singleton().findSchemaOrThrow(name, overload);
}
...
torch::Tensor moe_forward_fused_cutlass(
    ...
    auto& topk_ids_i64 = topk_ids;

    static auto remap_op = find_op("_moe_C::remap_hidden_states");
    remap_op.typed<void(torch::Tensor&, const c10::optional<torch::Tensor>&,
    ...
    cutlass_grouped_gemm_interface(remapped, w13, w13_scales, c10::nullopt,
        gemm1_out, efto, two_I, H, num_experts, true, false, false);

    static auto silu_op = find_op("_C::silu_and_mul");
    silu_op.typed<void(torch::Tensor&, torch::Tensor&)>().call(act_out, gemm1_out);

    cutlass_grouped_gemm_interface(act_out, w2, w2_scales, c10::nullopt,
        gemm2_out, efto, H, I, num_experts, true, false, false);

    static auto gather_op = find_op("_moe_C::moe_gather");
    gather_op.typed<void(torch::Tensor&, const torch::Tensor&, const torch::Tensor&,
...
// Full fused: topk + routed CUTLASS + shared expert (torch::mm FP16)
torch::Tensor moe_forward_full_fused_cutlass(
    torch::Tensor x, torch::Tensor logits,
    torch::Tensor w13, torch::Tensor w13_scales,
    torch::Tensor w2, torch::Tensor w2_scales,
    torch::Tensor shared_gu_w,     // [2*I_shared, H] fp16 (original N-major)
    torch::Tensor shared_gu_scale, // unused
    torch::Tensor shared_d_w,      // [H, I_shared] fp16 (original N-major)
    ...
    static auto topk_op = find_op("torch_ipex::topk_softmax", "moe");
    ...
    // ── Shared expert (torch::mm FP16, minimal overhead in C++) ──
    if (shared_gu_w.numel() > 0) {
        auto shared_gu = torch::mm(x, shared_gu_w.t());
        ...
        auto shared_down = torch::mm(shared_act, shared_d_w.t());
        auto gate_val = torch::sigmoid(torch::mm(x, shared_gate_w.t()));
