...
文生图和 ControlNet 工作流请直接使用 ComfyUI Template Browser 中维护的
官方版本。
...
Kontext 和 ControlNet 请直接使用 ComfyUI Template Browser 中维护的工作流。
...
请使用 FireRed 模型卡发布的工作流；本仓库不再保存其快照。
...
这些工作流（包括 Wan Animate）由 ComfyUI Template Browser 维护，不再复制到
本仓库。
...
T2V 和 I2V 工作流请直接使用 ComfyUI Template Browser 中维护的官方版本。
...
这些工作流由 ComfyUI Template Browser 维护，不复制到本仓库。
