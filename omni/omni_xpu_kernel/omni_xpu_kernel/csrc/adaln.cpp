...
template <typename T, bool RMS, typename Policy>
...
#define DISPATCH_ADALN(T, input_ptr, scale_ptr, shift_ptr, output_ptr)     \
    do {                                                                  \
        if (use_b580_candidate) {                                          \
            ...
            fused_adaln_kernel<T, false, device::B70KernelPolicy>(         \
                input_ptr, scale_ptr, shift_ptr, output_ptr, rows, hidden, \
                modulation_rows, row_repeat, static_cast<float>(eps),      \
                input.device());                                           \
        }                                                                  \
...
torch::Tensor fused_rms_adaln(
    ...
    auto output = torch::empty_like(input);
    if (input.scalar_type() == torch::kFloat32) {
        fused_adaln_kernel<float, true, device::GenericBmgKernelPolicy>(input.data_ptr<float>(), modulation_scale.data_ptr<float>(), modulation_shift.data_ptr<float>(), output.data_ptr<float>(), rows, hidden, modulation_rows, row_repeat, static_cast<float>(eps), input.device());
    } else if (input.scalar_type() == torch::kFloat16) {
        fused_adaln_kernel<fp16, true, device::GenericBmgKernelPolicy>(reinterpret_cast<const fp16*>(input.data_ptr()), reinterpret_cast<const fp16*>(modulation_scale.data_ptr()), reinterpret_cast<const fp16*>(modulation_shift.data_ptr()), reinterpret_cast<fp16*>(output.data_ptr()), rows, hidden, modulation_rows, row_repeat, static_cast<float>(eps), input.device());
    } else if (input.scalar_type() == torch::kBFloat16) {
        fused_adaln_kernel<bf16, true, device::GenericBmgKernelPolicy>(reinterpret_cast<const bf16*>(input.data_ptr()), reinterpret_cast<const bf16*>(modulation_scale.data_ptr()), reinterpret_cast<const bf16*>(modulation_shift.data_ptr()), reinterpret_cast<bf16*>(output.data_ptr()), rows, hidden, modulation_rows, row_repeat, static_cast<float>(eps), input.device());
    } else {
        TORCH_CHECK(false, "unsupported dtype");
    }
    return output;
