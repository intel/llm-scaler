...
def _modulation_reason(values: Any, x: torch.Tensor, segments: Any) -> str:
    if not isinstance(values, (tuple, list)) or len(values) != _EXPAND:
        return "adaln_outputs"
    expected_stride = (_EXPAND * _HIDDEN_SIZE, 1)
    rows = None
    for value in values:
...
def _run_fused(
    ...
    with context as (weight, bias):
        ...
        output = _omni_norm.rms_norm_modulate_b580(
            weight,
            x,
            scale,
            shift,
...
def apply():
    ...
    try:
        ...
        return False, "omni_xpu_kernel B580 RMS modulation is unavailable"
    if getattr(omni_package, "__xpu_target__", "") != "bmg":
        return False, "H3 RMS modulation is BMG-only"
    capability = getattr(candidate, "supports_rms_norm_modulate_b580", None)
    if not callable(capability) or not capability():
        return False, "native B580 H3 RMS modulation is unavailable"
    observed = omni_device.info(torch.xpu.current_device())
    if (
        observed.get("physical_bmg_sku") != "b580"
        or observed.get("sku_forced") is not False
    ):
        return False, "an unforced physical B580 is required"
    ...
    def patched(
        ...
        if h is None:
            raise RuntimeError(
                "B580 H3 norm2 modulation contract changed after routing: "
                f"{reason}"
            )
    ...
    log.info(
        "[OmniXPU] norm: patched physical-B580 MiniMax H3 RMS modulation"
    )
    return True, ""
